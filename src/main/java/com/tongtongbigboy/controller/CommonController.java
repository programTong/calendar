...
//        ImageIO.write(image,"PNG",);
    ...
    public Result checkAuthcode(@PathVariable String authCode, HttpServletRequest request){
        ...
        Application.log.info("checkcode_server: {}",checkcode_server);
        Application.log.info("authCode: {}",authCode);
        if (checkcode_server == null || ((String)checkcode_server).length() == 0) {
            return new Result(false, StatusCode.ERROR,"服务器验证码为空");
        }
        ...
        if (authCode.equalsIgnoreCase(checkcode_server.toString())){
            ...
            return new Result(true, StatusCode.OK,"验证码正确",json);
        }else{
            json.put("authcode_corrected",false);
            return new Result(false, StatusCode.ERROR,"验证码不正确",json);
        }
    }
